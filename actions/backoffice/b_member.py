...
from db.models import Member, AdministrativeDivision, Vehicle, GameMemberSource
...
class MemberListViewHandler(BaseHandler):
    """
    会员列表
    """

    @decorators.render_template('backoffice/members/list_view.html')
    ...
    async def get(self):
        ...
        if search_register_time:
            start = datetime.strptime(search_register_time, '%Y-%m-%d').replace(hour=0, minute=0, second=0)
            end = datetime.strptime(search_register_time, '%Y-%m-%d').replace(hour=23, minute=59, second=59)
            and_query_param.append({'register_datetime': {'$lt': end, '$gt': start}})
...
class MemberAddViewHandler(BaseHandler):
    """
    新增会员
    """

    @decorators.render_template('backoffice/members/add_view.html')
    @decorators.permission_required(PERMISSION_TYPE_MEMBER_MANAGEMENT)
    async def get(self):
        # 行政区划信息-联动
...
class MemberDetailViewHandler(BaseHandler):
    """
    会员详情
    """

    @decorators.render_template('backoffice/members/detail_view.html')
    @decorators.permission_required(PERMISSION_TYPE_MEMBER_MANAGEMENT)
    async def get(self):
...
class MemberEditViewHandler(BaseHandler):
    """
    编辑会员信息
    """

    @decorators.render_template('backoffice/members/edit_view.html')
    @decorators.permission_required(PERMISSION_TYPE_MEMBER_MANAGEMENT)
    async def get(self):
...
class MemberStatusEditViewHandler(BaseHandler):
    """
    修改车主状态
    """
...
class MemberDeleteViewHandler(BaseHandler):
    """
    删除会员信息
    """
...
class MemberChangeRacePrivilege(BaseHandler):
    """
    切换竞赛活动的特权
    """
    ...
    async def post(self):
        ...
        member_id = self.get_argument('member_id')
        target_status = self.get_argument('target_status')
        if member_id and target_status:
            try:
                member = await Member.get_by_id(member_id)
                member.race_privilege = target_status
                await member.save()
                res['code'] = 1
            except Exception:
                logger.error(traceback.format_exc())
...
URL_MAPPING_LIST = [
    url(r'/backoffice/member/list/', MemberListViewHandler, name='backoffice_member_list'),
    url(r'/backoffice/member/add/', MemberAddViewHandler, name='backoffice_member_add'),
    url(r'/backoffice/member/detail/', MemberDetailViewHandler, name='backoffice_member_detail'),
    url(r'/backoffice/member/edit/', MemberEditViewHandler, name='backoffice_member_edit'),
    url(r'/backoffice/member/status/', MemberStatusEditViewHandler, name='backoffice_member_status'),
    url(r'/backoffice/member/delete/', MemberDeleteViewHandler, name='backoffice_member_delete'),
    url(r'/backoffice/member/race/privilege/change/', MemberChangeRacePrivilege,
        name='backoffice_member_race_privilege_change')
]
