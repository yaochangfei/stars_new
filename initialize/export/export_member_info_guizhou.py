...
def statistic_member_info(race_cid: str, province_code, yesterday):
    ...
    temp_list = splitList(race_member_list, 10000)
    threads = []
    thread_num = 1
...
def export_all_race_member():
    ...
    for race in race_cursor:
        ...
        if race.cid == 'F742E0C7CA5F7E175844478D74484C29':
...
def test2():
    ...
    today = transform_date('20190708')
    yesterday = today.replace(hour=0, minute=0, second=0, microsecond=0)
    print(yesterday)
    # 六安 CA755167DEA9AA89650D11C10FAA5413 贵州 F742E0C7CA5F7E175844478D74484C29
    # 安徽 3040737C97F7C7669B04BC39A660065D
    info = RaceStatisticInfo.sync_find_one(
        {'race_cid': 'F742E0C7CA5F7E175844478D74484C29', 'daily_code': '2019-07-09'})
    ...
    # info.add_member_count = member_count
    # info.add_answer_times = answer_times
    # info.add_red_packet_count = red_packet_count
    # info.add_red_packet_amount = red_packet_amount
...
def test():
    today_code = '2019-07-09'
    # 六安 CA755167DEA9AA89650D11C10FAA5413 贵州 F742E0C7CA5F7E175844478D74484C29
    # 安徽 3040737C97F7C7669B04BC39A660065D
    history_info_list = RaceStatisticInfo.sync_find(
        {'race_cid': 'F742E0C7CA5F7E175844478D74484C29', 'daily_code': {'$lte': today_code}}).sort(
...
def test4():
    info = RaceStatisticInfo.sync_find_one(
        {'race_cid': 'F742E0C7CA5F7E175844478D74484C29', 'daily_code': '2019-07-09'})
    info.add_member_count = info.total_member_count - 138523
    info.add_answer_times = info.total_answer_times - 1464212
    info.add_red_packet_count = info.total_red_packet_count - 89253
    info.add_red_packet_amount = info.total_red_packet_amount - 60911.11000000813
    info.sync_save()
