...
from motorengine.utils import is_register
from settings import SMS_SIGNATURE

MOBILE_PATTERN = r"^\d{11}$"

SEND_MOBILE_UN = "shzywl-7"
SEND_MOBILE_PWD = "34d14d"
# SEND_MOBILE_UN = "shzywl-1"is_register
# SEND_MOBILE_PWD = "774ee2"
SEND_MESSAGE_HOST = "http://sms.800617.com:4400/sms/SendSMS.aspx"
...
def send_instant_sms(mobile, msg):
    ...
    # 存储发送历史
    if is_register(async=False):
        instant_sms = InstantSms(sms_server=SEND_MESSAGE_HOST, account=SEND_MOBILE_UN, post_dt=datetime.datetime.now(),
                                 status=status, mobile=mobile, content=msg, exception=exception)
...
if __name__ == '__main__':
    # register_database(settings.DB_NAME, settings.ADDRESS_LIST, user_name=settings.USER_NAME,
    #                   password=settings.USER_PASSWORD, async=False, default=True, **settings.DB_OPTIONS)
    print(send_instant_sms('15106139173', '你是宇宙第一强！！！'))
